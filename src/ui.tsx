...
function generateCss(collections: ColorCollection[]): string {
   ...
   collections.forEach(collection => {
      if (!collection.variables || collection.variables.length === 0) return;

      cssLines.push('');
      cssLines.push(`  /* ${collection.name} */`);
      ...
      collection.variables.forEach(variable => {
         ...
         if (isColorValue(value)) {
            cssValue = rgbaToHex(value);
         } else if (typeof value === 'number') {
            // Handle numeric values (like spacing, font sizes, etc.)
            if (variable.name.includes('spacing') ||
               variable.name.includes('size') ||
               variable.name.includes('width') ||
            ...
            // Handle string values (like font families)
            if (variable.name.includes('family')) {
               cssValue = `"${formatFontFamily(value)}"`;
            } else {
               cssValue = value;
            }
         } else if (isVariableAlias(value)) {
            ...
            const aliasName = variable.name;
            cssValue = `var(--${aliasName})`;
         }
...
function Plugin() {
   ...
   const downloadCssFile = (cssContent: string) => {
      const blob = new Blob([cssContent], { type: 'text/css' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = 'tailwind-variables.css';
   ...
   useEffect(() => {
      const handleSetVariables = (result: ColorCollection[]) => {
         console.log("UI received variables:", result);
         const cssOutput = generateCss(result);
         const jsOutput = generateJsMapping(result);
         ...
         downloadCssFile(cssOutput);

         // Notify success after processing
         emit("EXPORT_COMPLETE", true);
         figma.notify("Variables exported and downloaded!");
      };
